...
export default function AICoach({ 
  ...
  const handleSend = async (textToSend) => {
    ...
    // If user asked to log medication
    ...
    // Fallback baseline intelligent responder
    setTimeout(() => {
      ...
      if (lower.includes('ragweed') || lower.includes('pollen') || lower.includes('index')) {
  ...
  return (
    <div className="space-y-4 pb-12 max-w-4xl mx-auto flex flex-col h-[calc(100vh-8rem)]">
      {/* Context Loaded Header Badge */}
      <div className="bg-gradient-to-r from-teal-900 via-slate-900 to-slate-900 p-4 rounded-2xl border border-teal-800/40 text-white shadow-xs shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div>
            <h2 className="font-bold text-white text-base flex items-center gap-2">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${
                ...
                {isGeminiActive ? 'Gemini 1.5 Flash Connected' : 'Clinical Baseline Engine'}
              </span>
            </h2>
        ...
        <button
          ...
          className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-teal-300 border border-slate-700 rounded-xl text-xs font-semibold transition flex items-center gap-1.5 self-start sm:self-center cursor-pointer"
        >
          <Key className="w-3.5 h-3.5 text-teal-400" />
          <span>{isGeminiActive ? 'API Active' : 'Configure Gemini'}</span>
        </button>
      </div>
        ...
        {chatHistory.messages.map((msg) => {
          ...
          return (
            ...
            >
              ...
              {/* Chat Bubble */}
              <div className={`max-w-[82%] sm:max-w-[72%] space-y-1 ${isUser ? 'items-end' : 'items-start'}`}>
      ...
      <div className="space-y-3 shrink-0">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
          ...
          {chatHistory.suggestedQuickReplies.map((reply, idx) => (
            <button
              key={idx}
              onClick={() => handleSend(reply)}
              className="px-3 py-1.5 rounded-full bg-white hover:bg-teal-50 text-slate-700 hover:text-teal-900 border border-slate-200 hover:border-teal-300 text-xs font-medium transition whitespace-nowrap shadow-2xs flex items-center gap-1.5 shrink-0 cursor-pointer"
            >
        ...
        >
          <input
            ...
            placeholder="Ask AI Coach about symptoms, ragweed forecast, or dosing..."
